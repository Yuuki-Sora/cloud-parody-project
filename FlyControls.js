  ...
  class FlyControls extends THREE.EventDispatcher {
    constructor(cube, domElement) {
      ...
      this.update = function (delta) {
        ...
        scope.cube.translateX(scope.moveVector.x * moveMult);
        scope.cube.translateY(scope.moveVector.y * moveMult);
        scope.cube.translateZ(scope.moveVector.z * moveMult);
        scope.tmpQuaternion
          .set(
          ...
          .normalize();
        scope.cube.quaternion.multiply(scope.tmpQuaternion);

        if (
          lastPosition.distanceToSquared(scope.cube.position) > EPS ||
          8 * (1 - lastQuaternion.dot(scope.cube.quaternion)) > EPS
        ) {
          scope.dispatchEvent(_changeEvent);
          lastQuaternion.copy(scope.cube.quaternion);
          lastPosition.copy(scope.cube.position);
        }
      };
      ...
      this.updateRotationVector = function () {
        ...
        this.rotationVector.z =
          -this.moveState.rollRight + this.moveState.rollLeft; //console.log( 'rotate:', [ this.rotationVector.x, this.rotationVector.y, this.rotationVector.z ] );
      };
